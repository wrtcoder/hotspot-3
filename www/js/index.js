...
var AclCtrl, AppCtrl, FileCtrl, HotspotCtrl, HotspotListCtrl, MenuCtrl, MultiSelectCtrl, PermissionCtrl, SelectCtrl, VenueCtrl, VenueListCtrl, config, createHotspotCtrl, editHotspotCtrl, env, readHotspotCtrl,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

env = require('./env.coffee');

AppCtrl = function($rootScope, $scope, $http, platform, authService, model) {
  var fulfill;
  ...
  return $scope.$on('event:auth-loginRequired', function() {
    return platform.auth().then(fulfill, alert);
  });
};
...
FileCtrl = function($rootScope, $scope, $stateParams, $location, $ionicModal, model) {
  var FileView;
  FileView = (function() {
    ...
    FileView.prototype.home = function() {
      return $locPermissionCtrlation.url("file/todo/");
    };

    FileView.prototype.cd = function(folder) {
      if (folder == null) {
        folder = null;
      }
      if (_.isEmpty(folder || _.isNull(folder || _.isUndefined(folder)))) {
        return model.User.me().then((function(_this) {
          return function(user) {
            _this.model.path = user.username + "/";
            return _this.loadMore();
          };
        })(this))["catch"](alert);
      } else {
        this.model.path = folder;
        return this.loadMore();
      }
    };

    FileView.prototype.md = function(folder) {
      if (folder == null) {
        folder = 'New Folder/';
      }
      folder = new model.File({
        path: "" + this.model.path + folder
      });
      return foldPermissionCtrler.$save().then((function(_this) {
        return function() {
          return _this.model.add(folder);
        };
      })(this))["catch"](alert);
    };

    FileView.prototype.loadMore = function() {
    ...
    FileView.prototype.edit = function() {
      return $ionicModal.fromTemplateUrl('templates/file/edit.html', {
        scope: $scope
      }).then((function(_this) {
    ...
    FileView.prototype.remove = function(file) {
      return this.model.remove(file);
    };

    FileView.prototype.upload = function(files) {
      return _.each(files, (function(_this) {
        return function(local) {
          var remote;
          remote = (_.findWhere(_this.model.models, {
            name: local.name
          })) || new model.File({
            path: "" + _this.model.path + local.name
          });
          return remote.$save({
            file: local
          }).then(function() {
            return _this.model.add(remote);
          })["catch"](alert);
        };
      })(this));
    };
  ...
  if (_.isUndefined($scope.model)) {
    $scope.model = new model.File({
      path: $stateParams.path
    });
    $scope.model.$fetch();
  }
  $scope.controller = new FileView({
    model: $scope.model
  });
  $scope.$watchCollection('files', function(newfiles, oldfiles) {
    if (((newfiles != null ? newfiles.length : void 0) != null) && (newfiles != null ? newfiles.length : void 0) !== (typeof oldfiles === "function" ? oldfiles(length) : void 0)) {
      return $scope.controller.upload(newfiles);
    }
  });
  return $scope.$watchCollection('model.tags', function(newtags, oldtags) {
    if ((newtags != null ? newtags.length : void 0) !== (oldtags != null ? oldtags.length : void 0)) {
...
SelectCtrl = function($scope, $ionicModal) {
  var SelectView;
  SelectView = (function() {
    function SelectView() {}

    SelectView.prototype.select = function(name, model1, collection) {
      this.name = name;
      this.model = model1;
      this.collection = collection;
      return $ionicModal.fromTemplateUrl('templates/permission/select.html', {
        scope: $scope
      }).then((function(_this) {
        return function(modal) {
          _this.modal = modal;
    ...
    SelectView.prototype.ok = function() {
      $scope.$emit(this.name, this.model);
      return this.modal.remove();
    };

    SelectView.prototype.cancel = function() {
      _.extend(this.model, this.model.previousAttributes);
      return this.modal.remove();
    };
  ...
  return $scope.controller = new SelectView();
};

MultiSelectCtrl = function($scope, $ionicModal) {
  var MultiSelectView;
  MultiSelectView = (function() {
    function MultiSelectView() {}

    MultiSelectView.prototype.select = function(name, model1, collection) {
      this.name = name;
      this.model = model1;
      this.collection = collection;
      return $ionicModal.fromTemplateUrl('templates/permission/multiselect.html', {
        scope: $scope
      }).then((function(_this) {
        return function(modal) {
          _this.modal = modal;
    ...
    MultiSelectView.prototype.selected = function(value) {
      return _.contains(this.model, value);
    };

    MultiSelectView.prototype.ok = function() {
      this.model = _.map($(this.modal.$el).find('input:checked'), function(el) {
        return el.name;
      });
      $scope.$emit(this.name, this.model);
      return this.modal.remove();
    ...
    MultiSelectView.prototype.cancel = function() {
      _.extend(this.model, this.model.previousAttributes);
      return this.modal.remove();
    };
  ...
  return $scope.controller = new MultiSelectView();
};

PermissionCtrl = function($rootScope, $scope, $ionicModal, model) {
  var PermissionView;
  PermissionView = (function() {
    PermissionView.prototype.modelEvents = {
      userGrp: 'update',
      fileGrp: 'update',
      action: 'update'
    };

    function PermissionView(opts) {
      if (opts == null) {
        opts = {};
      }
    ...
    PermissionView.prototype.update = function(event, value) {
      return this.model[event.name] = value;
    };

    PermissionView.prototype.save = function() {
      return this.model.$save()["catch"](alert);
    };

    return PermissionView;

  })();
  return $scope.controller = new PermissionView({
    model: $scope.model
  });
};

AclCtrl = function($rootScope, $scope, model) {
  var AclView;
  AclView = (function() {
    function AclView(opts) {
      if (opts == null) {
        opts = {};
      }
      _.each(this.events, (function(_this) {
    ...
    AclView.prototype.loadMore = function() {
      return this.collection.$fetch().then(function() {
        return $scope.$broadcast('scroll.infiniteScrollComplete');
      })["catch"](alert);
    };

    AclView.prototype.add = function() {
      return this.collection.add(new model.Permission({
        userGrp: '',
        fileGrp: '',
        action: []
      }));
    ...
    AclView.prototype.remove = function(perm) {
      return this.collection.remove(perm);
    };
  ...
  return $scope.controller = new AclView({
